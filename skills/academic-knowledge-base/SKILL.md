...
- **技能位置**：与本插件同目录下的 `Karpathy LLM Wiki/`
...
- **技能位置**：与本插件同目录下的 `global-biblio-base/`
- **版本**：v3.1
...
```
~/.smartlib/academic-kb/sessions/{slug}_{yyyymmdd}/
...
Key 写入 `~/.smartlib/academic-kb/vectors/vector_config.json` 的 `api_key` 字段。
...
|------|------|
| 零越权文件访问 | 所有操作限定在 `~/.smartlib/academic-kb/` 内 |
